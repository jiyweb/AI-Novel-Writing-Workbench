...
- 模型设置新增「默认文本模型」与「默认生图模型」：从配置好的厂商和模型里各选一个设为全局默认，确认后写作、规划、审校等全部文字任务与角色、封面、漫画等图片任务都按默认模型执行；个别场景需要换模型时，仍可在对应页面单独选择。
- 生图厂商卡片新增「高级维护」：可直接增删该厂商的生图模型（标签式列表、一键删除、输入名称添加），清空后该厂商不再提供生图选项；支持一键恢复厂商默认生图模型，自定义厂商可在卡片上直接删除。
- 漫画工作台的生图偏好「跟随主程序」改为跟随主程序的默认生图模型：主程序设了哪个默认生图模型，漫画画面生成默认就用哪个。

#### 优化

- 模型设置弹窗的文本模型分区提供默认模型快捷选择，配好厂商后不用离开弹窗即可完成全局默认切换。
- 厂商配置弹窗按打开入口分区呈现：从「生图模型」分区打开只显示连接凭据与生图模型，从「文本模型」分区打开只显示连接凭据与文本模型，互不混杂。
