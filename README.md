...
- 漫画工作台升级为一站式漫画制作流程：从小说原文或一句灵感出发，依次完成内容导入、形态画风选择、智能分镜、角色场景库、台词编排、描述词组装、画面生成与成品导出；所有项目数据与生成的图片都保存在本机浏览器中，隐私可控。
- 内容导入支持三种方式：TXT 文件与粘贴的小说原文直接进入制作流程；输入一句灵感关键词即可让 AI 扩写出人物、场景与完整正文，并可叠加「爽文打脸」「甜宠互动」「悬疑反转」「情绪共鸣」四种叙事模板控制故事节奏。
- 漫画形态提供条漫、四格、多格页、短剧画面等 7 种选择，配合内置画风预设使用；色调、线条、画质、光影均可微调，生成试画确认效果后再进入制作，个人风格可保存复用。
- 智能分镜按原文自动切分画面：5 档密度可调，高潮与对话密集处自动加密；支持手动拆分、合并、增删、拖拽排序，还可以让 AI 补充镜头类型、情绪与动作概述，让画面语言更专业。
- 角色与场景库由 AI 从原文提取生成：上传参考图、合并同名角色、编辑外貌与服饰描述后，全书画面统一沿用同一套设定，保证角色与场景前后一致。
- 台词自动提取并按漫画形态适配呈现样式（对话气泡、底部字幕、聊天框）：支持修改文本、更换说话人、拖拽气泡定位与调整字号宽度。
- 每格画面的描述词按公式自动组装，支持单镜分段微调、一键重置与个人模板保存，改完立刻可用于重新生成。
- 画面生成提供草稿与高清两档：批量生成支持断点续传，失败自动重试，单镜可单独重绘或换档重出。
- 成品导出提供三种方式：单张 PNG 按章节归档打包、条漫长图（可把台词按编排位置合成进画面）、以及整包 ZIP 下载。
- 爆款运营支持：为抖音、小红书、微博生成贴合平台风格的标题、发布正文与话题标签，一键复制即可发布；选择发布平台后还会得到更合适的漫画形态建议与运营节奏提示。
- 智能分镜新增 AI 模式：优先让文本模型通读整章并按剧情与对话节奏划分镜头，AI 未配置或生成失败时自动回退本地规则，保证分镜总能顺利完成。
- 画面生成支持把台词、旁白与拟声词直接绘制进图片：开启后对话内容随画面一体产出，图文不分家；可随时关闭，重新生成后仅保留气泡位置提示。
- 描述词环节新增角色/场景绑定检查：未绑定出场角色或场景的分镜会醒目提示并提供一键修复（按台词说话人、场景信息自动补齐），也可在单镜编辑里手动勾选；无人物的空镜可标记后跳过。
- 漫画工作台的 AI 设置支持从主程序一键导入模型选择：文本模型与生图模型的厂商、模型、接口地址自动回填，API Key 留空待补填，密钥依旧只存本机。

#### 优化

- 制作流程每一步完成后都显示明确的「下一步」按钮与就绪提示：导入内容、选择形态画风、生成分镜、建好角色场景、完成台词与描述词后，顺着按钮即可进入下一环节。
- 形态选择页的漫画形态示意图形等比缩小，一屏可见更多形态，减少来回滚动。
- 画面生成页改为多列卡片布局：分镜画面按行排列，一屏浏览更多格子，长章节滚动依然流畅。
- 画面生成页工具条可直接切换生图模型与该模型下的版本，换模型重出图无需离开当前页面。
- 修改分镜、角色库或画风后，受影响的台词、描述词与画面会明确标记「待更新」，并给出待更新数量提示；可一键按依赖顺序重跑台词与描述词环节，画面是否重新生成由自己决定，避免悄悄消耗生图额度。
- 分镜编辑器与原文预览对长章节做了滚动优化，几千字原文分镜时依然流畅。

#### 修复

- 修复短剧角色立绘数据加载失败导致形象无法显示的问题。
