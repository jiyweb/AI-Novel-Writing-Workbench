...
- 模型设置新增「火山方舟」内置厂商：文本创作可使用豆包模型，角色形象、小说封面、漫画生图可使用豆包 Seedream 模型并支持参考图；同一把 API Key 同时用于文本与生图。
- 生图模型设置新增「GrsAI 生图」内置厂商，可使用 gpt-image、nano-banana 等模型生成角色图、封面和漫画图。该平台只提供生图能力，配置后只会出现在生图模型分区；默认走全球节点，国内访问较慢时可在配置里切换国内节点。
- 漫画工作台的项目卡片新增「删除项目」入口。确认删除后，该项目的全部话数、角色、角色素材、场景、分格、导出任务以及已生成的图片都会被永久清理，不会在本机留下失效文件。
- 漫画画风支持自定义：新建漫画项目和项目内的画风设置都可以选择「自定义画风」，用自己的话描述想要的风格流派、线条、配色、光影和参考作品；角色形象、场景设定图和每一格画面都会按这段描述统一生成。

#### 优化

- 模型设置拆成「文本模型」和「生图模型」两个分区：文本模型分区保留厂商连接、余额、思考开关等全部管理能力；生图模型分区只展示当前使用的生图模型和配置入口，没有可用生图模型时会给出明确的配置引导。添加厂商仍只需填写一次 API Key 和地址，文本、生图共用同一套凭据。
- 角色形象、小说封面、漫画格子等图片生成入口的提示文案统一为「生图模型」，并引导到模型设置完成配置，避免「图片模型」「图像模型」多种叫法造成困惑。
- 漫画工作台顶部可直接选择生图厂商和该厂商下已配置的生图模型（如 nano-banana、gpt-image 等不同模型），切换后角色设计稿、场景设定图和格子图都会用新选择生成；选择的模型会保存在模型设置中，全书的图片生成共用。
- 漫画项目页头部重新整理为「标题与进度 + 创作设置」两层：话数、总格数、角色一眼可见；漫画形态、画风、生图厂商和模型集中在同一行调整，形态与画风选择弹层点击空白处即可关闭，不再只有取消按钮。
- 漫画制作各步骤补齐下一步引导：格子图和导出页在还没有大纲或分格脚本时，会说明操作顺序并提供按钮直接回到分话大纲；按钮暂时不可用时会提示原因；生成分格脚本等待期间会提示大约需要的时间。
- 左侧导航隐藏未开放的短剧工作台入口，只保留当前可用的创作入口。
- Windows 桌面版更新至 0.4.26，包含漫画项目删除、模型设置分区、自定义画风、漫画工作台体验优化与本次全部改动；以后每次重新打包都会自动递增版本号，无需手动修改。

#### 修复

- 修复漫画项目内点击返回「漫画工作台」时页面异常跳转或报错的问题，返回会在应用内直接回到漫画项目列表。
- 修复话数较多、分格较密时生成分格脚本经常失败的问题：生成时会按目标格数自动申请足够的输出长度并延长等待时间；遇到输出长度有限的模型时，会自动改用更精简的字段写法，保证整话分格一次性完整生成。
