...
- 内容导入自动整理原稿：自动识别章节标题（纯数字、「第X章」、英文 Chapter 等多种写法），把前言、导语、作者的话等非正文内容移出分镜范围，并清理广告推广行与隐形符号，避免切出大量空白分镜；导入预览会显示识别结果。
- 角色卡新增「AI 补全设定」：按章节原文把发型发色、眼睛、脸型、体型气质与主色调补全成完整人设，原文没有明写的细节会按角色身份合理设计。
- 角色卡新增人物设计图：固定 7 张参考图（全身正面、四分之三侧、正侧、背面 + 面部正面、四分之三侧、正侧特写），可一键生成全套、单张重画或放大查看；角色设定修改后会提示重新生成，保证全书人物长相一致。
- 场景卡新增概念图：一键生成场景全貌参考图（无人物无文字），支持重新生成与放大预览；场景设定修改后同样提示更新。
- 资产区新增「漫画规则」：集中调整画风参数（线条、画质、光影、色调、追加画风关键词）、分镜密度（可一键应用到全部章节），并查看台词呈现方式与描述词模板来源；画风参数保存后受影响的画面自动标记待更新。
- 漫画项目页新增「AI 实况」入口；图片生成任务的全过程也会进入 AI 实况：提交、生成中、保存、成功、失败、自动重试与取消都能看到。
- 模型设置弹窗新增「生图模型」分区：与文本模型并列切换，可在弹窗里选择厂商、填写 API Key 与接口地址、挑选或填写生图模型并保存，封面、漫画等图片生成立刻使用。

#### 优化

- 模型设置弹窗的生图配置与系统设置「模型与厂商」共用同一套流程：支持生图的厂商卡片显示「可用/待配置」与当前生图模型，未配置的厂商可直接点开补齐 API Key 与生图模型，也可以添加任意 OpenAI 兼容的自定义服务。
- 模型设置弹窗把与内置厂商同名的自定义配置合并为一张厂商卡片（例如重复出现的「火山方舟」），原有模型与 Key 继续生效，重新检测时可直接沿用，无需重复粘贴密钥。
- 漫画工作台的 AI 全部直接使用主程序「模型设置」：灵感扩写、角色场景提取、智能分镜等文本创作与全部画面生成共用主程序已配置的模型，无需再为漫画单独填写 API Key 或接口地址，一次配置全书通用；主程序尚未配置模型时会明确提示并给出一键前往设置的入口。
- 漫画工作台的「AI 模型」面板改为状态总览：一眼看到主程序文本与生图模型的就绪情况，可直接跳转主程序模型设置；生图模型列表来自主程序已配置的生图厂商，也可以为漫画单独指定一个生图模型偏好（默认跟随主程序）。
...
- 台词提取升级为三类文字：对白、旁白、内心独白；传统漫画每一格都保证有文字（空镜也会配过渡旁白），旁白用字幕条、内心独白用虚线气泡呈现，剧情更连贯。
- 漫画工作区页面高度加长，长章节编辑减少滚动；所有漫画 AI 调用在实况中显示清晰的中文任务名。

#### 修复

- 修复漫画工作台画风试画失败并提示 "Base character was not found." 的问题：漫画分镜的生成任务可以正常找到归属并出图。
- 修复短剧角色立绘数据加载失败导致形象无法显示的问题。
- 漫画「台词绘制到画面」开关现在与画面联动：切换后已有画面自动标记待更新，重新生成时描述词会按新开关自动重组，画面重绘后正确带上或去掉台词文字。
- 漫画生图在本地取消、等待超时或连接中断后会同步终止服务端的生成任务，避免「继续生成」重复出图、重复消耗生图额度。
- 修复自定义画风保存后仍能同时勾选内置画风的问题：画风只能单选，选定的风格贯穿角色、场景与画面。
- 修复分镜、台词、描述词生成后重新进入页面偶尔丢失的问题（写入冲突导致内容被覆盖）。
